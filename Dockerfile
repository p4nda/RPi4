...
# Download official RPi firmware: stable | next
...
#FROM docker.io/almalinux:9.3-minimal-20231124 as build
ARG PROJECT_URL
ARG BRANCH
ARG GIT_BRANCH
ARG ARCH
...
RUN microdnf -y update && \
    microdnf -y install vim mc git wget zip findutils epel-release

# In CentOS stream9 and UBI 9 image, curl-minimal is already installed in the image.
# RUN microdnf -y install curl # Rocky only
...
# Get latest EDK2 release https://github.com/tianocore/edk2/tags
# a) Copy contents of locally downloaded RPi4 EDK2 repository with all submodules
# COPY RPi4-1.36-beta-update-20240224/ .
...
#FROM docker.io/almalinux:9.3-minimal-20231124 as final
ARG CONTAINER_NAME
ARG ARCHIVE_NAME
ARG BRANCH
ARG GIT_BRANCH
...
RUN set -exou pipefail; \
    microdnf -y update && microdnf -y install shadow-utils policycoreutils && \
    ...
    microdnf remove -y shadow-utils policycoreutils && microdnf clean all
...
RUN curl -O -L $RPI_FIRMWARE_URL/raw/$START_ELF_VERSION/boot/fixup4.dat && \
    ...
    curl -O -L $RPI_FIRMWARE_URL/raw/$DTBO_VERSION/boot/overlays/disable-emmc2.dtbo && \
